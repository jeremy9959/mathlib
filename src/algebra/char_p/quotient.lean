...
{ have := ideal.quotient.eq_zero_iff_mem.2
          (set_like.mem_coe.mp $ ideal.subset_span $ set.mem_singleton (p : R)),
  rwa [map_nat_cast] at this },
...
lemma quotient' {R : Type*} [comm_ring R] (p : ℕ) [char_p R p] (I : ideal R)
  ...
  char_p (R ⧸ I) p :=
⟨λ x, begin
  rw [←cast_eq_zero_iff R p x, ←@map_nat_cast R _ _ _ _ _ (ideal.quotient.mk I)],
