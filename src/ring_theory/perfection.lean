...
char_p.quotient O p $ mt hv.one_of_is_unit $
(@map_nat_cast O K _ _ _ _ (algebra_map O K) p).symm ▸ hvp.1
...
begin
  ...
  obtain ⟨r, rfl⟩ := ideal.quotient.mk_surjective x,
  rw [pre_val_mk h, ←@map_nat_cast O K _ _ _ _ (algebra_map O K) p, hv.le_iff_dvd] at hp,
...
begin
  ...
  refine rpow_le_rpow_of_exponent_ge hvp (@map_nat_cast O K _ _ _ _ (algebra_map O K) p ▸ hv.2 _) _,
